...
<head>
   ...
   <title>Dashboard - Sucursal Palmeras</title>
   <script>
...
<body class="bg-gray-100">
   <nav class="bg-white shadow">
       <div class="max-w-7xl mx-auto px-4">
           <div class="flex h-16 justify-between items-center">
               <h1 class="text-xl font-bold">Sucursal Palmeras</h1>
  ...
  async function loadData(day = 0) {
        try {
            const response = await fetch(`${BASE_URL}/../src/Controllers/DashboardController.php?action=stats&sucursal=palmeras&periodo=dia&day=${day}`);
            ...
            if (data.success) {
                ...
                const ordersResponse = await fetch(`${BASE_URL}/../src/Controllers/DashboardController.php?action=orders&sucursal=palmeras&day=${day}`);
...
function updateTable(orders) {
  ...
  async function updateField(id, field, element) {
    ...
    async function updateStatsTotals() {
        try {
            const response = await fetch(`${BASE_URL}/../src/Controllers/DashboardController.php?action=stats&sucursal=palmeras&periodo=dia&day=${currentDay}`);
            const data = await response.json();
  ...
  function startPolling() {
    ...
    async function checkUpdates() {
        try {
            const response = await fetch(`${BASE_URL}/fetch-updates.php?sucursal=palmeras&last_update=${lastUpdate}&day=${currentDay}`);
            const data = await response.json();
